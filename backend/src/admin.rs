...
fn get_image_cache() -> Option<Html<String>> {
    ...
    for rfile in previews::cache_path().read_dir().ok()? {
        if let Ok(file) = rfile {
            ...
            let res: usize = file.file_name().to_string_lossy().split("_").next().and_then(
                |res_str| res_str.parse().ok()
            ).unwrap_or(0);
            if res == 0 {
                warn!("Unknwon cached file resolution of \"{:?}\"", file.file_name());
            }
    ...
    "#, total_size = total_size, total_count = total_count);
...
fn cleanup_image_cache(max_size: Option<u64>) -> Option<Html<String>> {
    ...
    info!("Image cache cleanup, keeping newest {} mb", max_size / (1024 * 1024));
    ...
    let mut all_files: Vec<CacheFile> = previews::cache_path().read_dir().ok()?.filter_map(|rfile| {
    ...
    let page = if successful {
        ...
        "#, del_len = delete.len(), rem_len = retain.len(), rem_size = retained_size / (1024 * 1024))
    } else {
        r#"
        <html>
            <body>
